...
namespace MQTTnet.Internal
{
    public sealed class AsyncLock : IDisposable
    {
        ...
        public AsyncLock()
        {
            ...
            _releaser = Task<IDisposable>.Factory.StartNew(()=> { return new Releaser(this); });// ((IDisposable)new Releaser(this));
        }
        ...
        public Task<IDisposable> WaitAsync(CancellationToken cancellationToken)
        {
            ...
            lock (_syncRoot)
            {
                task = Task.Factory.StartNew(() => { _semaphore.Wait(); });//.WaitAsync(cancellationToken);
            }
            ...
            Func<Task, IDisposable> p = (_) => { return _releaser.Result; };
            return task.ContinueWith<IDisposable>(p, cancellationToken, TaskContinuationOptions.ExecuteSynchronously, TaskScheduler.Default);
        }

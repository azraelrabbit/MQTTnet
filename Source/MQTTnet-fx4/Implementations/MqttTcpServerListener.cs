    ...
    public sealed class MqttTcpServerListener : IDisposable
    {
        ...
        public bool Start(bool treatErrorsAsWarning, CancellationToken cancellationToken)
        {
            try
            {
                ...
                Task.Factory.StartNew(() => AcceptClientConnectionsAsync(cancellationToken), cancellationToken).RunInBackground(_logger);
        ...
        async Task AcceptClientConnectionsAsync(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    Task.Factory.StartNew(() => TryHandleClientConnectionAsync(clientSocket), cancellationToken).RunInBackground(_logger);
                }

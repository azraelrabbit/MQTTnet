    ...
    public sealed class MqttChannelAdapter : Disposable, IMqttChannelAdapter
    {
        ...
        public async Task ConnectAsync(TimeSpan timeout, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                      _channel.ConnectAsync(cancellationToken).Wait();//.ConfigureAwait(false);
                }
                else
                {
                    MqttTaskTimeout.WaitAsync(t => _channel.ConnectAsync(t), timeout, cancellationToken).Wait();//.ConfigureAwait(false);
                }
            }
        ...
        public async Task DisconnectAsync(TimeSpan timeout, CancellationToken cancellationToken)
        {
            ...
            try
            {
                if (timeout == TimeSpan.Zero)
                {
                      _channel.DisconnectAsync(cancellationToken).Wait();//.ConfigureAwait(false);
                }
                else
                {
                      MqttTaskTimeout.WaitAsync(
                        t => _channel.DisconnectAsync(t), timeout, cancellationToken).Wait();//.ConfigureAwait(false);
                }
            }
            catch (Exception exception)
            {
        ...
        public async Task SendPacketAsync(MqttBasePacket packet, CancellationToken cancellationToken)
        {
            ...
            using ( var task= _syncRoot.WaitAsync(cancellationToken))//.ConfigureAwait(false))
            {
        ...
        static void WrapAndThrowException(Exception exception)
        {
            ...
            {
                //if ((uint)comException == ErrorOperationAborted)
                //{
                    throw new OperationCanceledException();
                //}
            }

...
namespace MQTTnet.Extensions.ManagedClient
{
    public class ManagedMqttClient : Disposable, IManagedMqttClient
    {
        ...
        public async Task StartAsync(IManagedMqttClientOptions options)
        {
            ...
            //_maintainConnectionTask = Task.Factory.StartNew(() => MaintainConnectionAsync(cancellationToken), cancellationToken);
        ...
        async Task TryMaintainConnectionAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (connectionState == ReconnectionResult.NotConnected)
                {
                    ...
                    await TaskEx.Delay(Options.AutoReconnectDelay, cancellationToken).ConfigureAwait(false);
                    //await Task.Factory.StartNew(() => { Thread.Sleep(Options.AutoReconnectDelay); }).ConfigureAwait(false);
        ...
        async Task PublishSubscriptionsAsync(TimeSpan timeout, CancellationToken cancellationToken)
        {
            ...
            //while (await _subscriptionsQueuedSignal.WaitAsync(GetRemainingTime(endTime), cancellationToken).ConfigureAwait(false))
                while (  _subscriptionsQueuedSignal.Wait(GetRemainingTime(endTime)))
            {
        ...
        async Task<ReconnectionResult> ReconnectIfRequiredAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var result = InternalClient.ConnectAsync(Options.ClientOptions, cancellationToken).Result;
        ...
        void StartPublishing()
        {
            ...
            //Task.Factory.StartNew(() => PublishQueuedMessagesAsync(cancellationToken), cancellationToken).RunInBackground(_logger);

        ...
        public async Task<MqttClientConnectResult> ConnectAsync(IMqttClientOptions options, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                using (var combined = CancellationTokenSource.CreateLinkedTokenSource(backgroundCancellationToken, cancellationToken))
                {
                    ...
                    //_publishPacketReceiverTask = Task.Factory.StartNew(() => ProcessReceivedPublishPackets(backgroundCancellationToken), backgroundCancellationToken);
                    ...
                    //_packetReceiverTask = Task.Factory.StartNew(() => TryReceivePacketsAsync(backgroundCancellationToken), backgroundCancellationToken);
                ...
                {
                    //_keepAlivePacketsSenderTask = Task.Factory.StartNew(() => TrySendKeepAliveMessagesAsync(backgroundCancellationToken), backgroundCancellationToken);
        ...
        async Task DisconnectCoreAsync(Task sender, Exception exception, MqttClientConnectResult connectResult, bool clientWasConnected)
        {
            ...
            try
            {
                ...
                var keepAliveTask = WaitForTaskAsync(_keepAlivePacketsSenderTask, sender);
                  ...
                  Task.WaitAll(receiverTask, publishPacketReceiverTask, keepAliveTask);//.ConfigureAwait(false);
            }
            ...
            finally
            {
                ...
                if (disconnectedHandler != null)
                {
                    ...
                    //Task.Factory.StartNew(() => disconnectedHandler.HandleDisconnectedAsync(new MqttClientDisconnectedEventArgs(clientWasConnected, exception, connectResult, _disconnectReason))).RunInBackground(_logger);
                    TaskEx.Run(() => disconnectedHandler.HandleDisconnectedAsync(new MqttClientDisconnectedEventArgs(clientWasConnected, exception, connectResult, _disconnectReason))).RunInBackground(_logger);
                }
            }
        }
        ...
        async Task TrySendKeepAliveMessagesAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    // packet.
                    //await Task.Factory.StartNew(() => { Thread.Sleep(250); },cancellationToken);//.Delay(250, cancellationToken).ConfigureAwait(false);

                    await TaskEx.Delay(250, cancellationToken).ConfigureAwait(false) ;//.Delay(250, cancellationToken).ConfigureAwait(false);

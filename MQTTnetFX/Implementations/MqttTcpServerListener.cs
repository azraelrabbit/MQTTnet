    ...
    public sealed class MqttTcpServerListener : IDisposable
    {
        ...
        public bool Start(bool treatErrorsAsWarning, CancellationToken cancellationToken)
        {
            try
            {
                ...
                TaskEx.Run(() => AcceptClientConnectionsAsync(cancellationToken), cancellationToken).RunInBackground(_logger);
        ...
        async Task AcceptClientConnectionsAsync(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    TaskEx.Run(() => TryHandleClientConnectionAsync(clientSocket), cancellationToken).RunInBackground(_logger);
                }
                ...
                catch (Exception exception)
                {
                    ...
                    await TaskEx.Delay(TimeSpan.FromSeconds(1), cancellationToken).ConfigureAwait(false);

                    //await Task.Factory.StartNew(() => { Thread.Sleep(TimeSpan.FromSeconds(1)); }, cancellationToken);// TimeSpan.FromSeconds(1), cancellationToken).ConfigureAwait(false);
                }
            }
        }

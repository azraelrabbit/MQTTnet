        ...
        async Task DisconnectInternalAsync(Task sender, Exception exception, MqttClientAuthenticateResult authenticateResult)
        {
            ...
            finally
            {
                ...
                if (disconnectedHandler != null)
                {
                    ...
                    TaskEx.Run(() => disconnectedHandler.HandleDisconnectedAsync(new MqttClientDisconnectedEventArgs(clientWasConnected, exception, authenticateResult, _disconnectReason))).RunInBackground(_logger);
                }
            }
        }
        ...
        async Task TrySendKeepAliveMessagesAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    // packet.
                    await TaskEx.Delay(TimeSpan.FromMilliseconds(100), cancellationToken).ConfigureAwait(false);
                }
            }
            catch (Exception exception)
            {
